...
$grouptenantmasid = $_GET['grouptenantmasid'];
    ...
    ////$custom = array('msg'=>$grouptenantmasid,'s'=>"Success");	
     ...
     if ($result != false)
{
	while($row = mysql_fetch_assoc($result))
	{
		...
		$custom = array('msg'=>"<font color='red'> Alert: Tenancy canceled Successfully. </font>",'s'=>"Success");	
		...
		if($result1 != null)
		{
			...
			if($num > 0 )
			{
                            ...
                            ////delete from waiting list
                            //$sqldeltenantmasid = "delete from mas_tenant where tenantmasid=".$tenantmasid;				
                            //mysql_query($sqldelofflet);
			}
			else
			{
