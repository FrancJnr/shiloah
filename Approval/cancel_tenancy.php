	...
	oTable = $('#example').dataTable({
       ...
       $('[id^="btnFinalize"]').live('click', function() {
	    ...
	    if (r == true)
	    {
                ...
                var url="save_cancel_tenancy.php?action=finalize&grouptenantmasid="+$(this).attr('val');
	    ...
	    {
                var $a = $(this).attr('name');
                var url="save_cancel_tenancy.php?action=cancel&grouptenantmasid="+$(this).attr('val');
                var dataToBeSent = $("form").serialize();
		$.getJSON(url,dataToBeSent, function(data){
                    $.each(data.error, function(i,response){
			if(response.s == "Success")
		...
		<?php		
		 ...
		 if($result != null)
		 {
			...
			$tr =  "<tr>
				...
				<td>".$row['shopcode']."</td>
				<td>".$row['doc']."</td>
			        <td class='center'><button type='button' id=btnCancel$i name='".$i."'  val='".$grpmasid."'>Cancel Tenancy</button></td>";
			echo $tr;
			$i++;
		    }
		 }
