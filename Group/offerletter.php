	...
	if (k >0)
	{
		...
		$.getJSON(url,dataToBeSent, function(data){
			...
			if(response.s == "Success")
			    ...
			    $('#grouptenant').empty(); loadTenantDraft("loadTenantDraft");
			    $('#grouptenantlist').empty();loadgrouptenant("loadgrouptenant");
                            // alert(response.tenanttype);  
                    if(response.msg=="Data Grouped Successfully"){  
                       
                                         switch(response.tenanttype){
                                          
                                           case "SIMPLE TENANT" :
                                               
                                                var a = confirm("Simple Tenant Grouped Successfully\nWould You Like to Proceed to Print Simple Agreement? ");
                                            if (a== true)
                                            {
                                              parent.top.$('div[name=masterdivtest]').html("<iframe  src='reports-pms/rpt_simple_agreement.php?action=new' id='the_iframe' scrolling='yes' width='100%'></iframe>");   
                                            }else{
                                            $('#divContent').html(response.divContent);
                                            $('#grouptenant').empty(); loadTenantDraft("loadTenantDraft");
                                            $('#grouptenantlist').empty();loadgrouptenant("loadgrouptenant");
                                           ...
                                           case "LEASE TENANT" :
                                             var a = confirm("Lease Tenant Grouped Successfully\nWould You Like to Proceed to Print Offerletter? ");
                                            if (a== true)
                                            {
                                              parent.top.$('div[name=masterdivtest]').html("<iframe  src='reports-pms/print_offerletter.php?action=new' id='the_iframe' scrolling='yes' width='100%'></iframe>");   
                                            }else{
                                              $('#divContent').html(response.divContent);
                                             $('#grouptenant').empty(); loadTenantDraft("loadTenantDraft");
                                             $('#grouptenantlist').empty();loadgrouptenant("loadgrouptenant");
                                            ...
                                            case "WASHROOM TENANT":
                                             var a = confirm("Washroom Tenant Grouped Successfully\nWould You Like to Proceed to Print Washroom Agreement? ");
                                            if (a== true)
                                            {
                                              parent.top.$('div[name=masterdivtest]').html("<iframe  src='reports-pms/rpt_washroomkiosk_agreement.php?action=new' id='the_iframe' scrolling='yes' width='100%'></iframe>");   
                                            }else{
                                              $('#divContent').html(response.divContent);
                                            $('#grouptenant').empty(); loadTenantDraft("loadTenantDraft");
                                            $('#grouptenantlist').empty();loadgrouptenant("loadgrouptenant");
                                            ...
                                            break;
                                            case "BANK TENANT" :
                                             var a = confirm("Bank Tenant Grouped Successfully\nWould You Like to Proceed to Print Bank Agreement? ");
                                            if (a== true)
                                            {
                                              parent.top.$('div[name=masterdivtest]').html("<iframe  src='reports-pms/rpt_bank_agreement.php?action=new' id='the_iframe' scrolling='yes' width='100%'></iframe>");   
                                            }else{
                                              $('#divContent').html(response.divContent);
                                            $('#grouptenant').empty(); loadTenantDraft("loadTenantDraft");
                                            $('#grouptenantlist').empty();loadgrouptenant("loadgrouptenant");
                                            ...
                                            if (a== true)
                                            {
                                              parent.top.$('div[name=masterdivtest]').html("<iframe  src='reports-pms/print_offerletter.php?action=new' id='the_iframe3' scrolling='yes' width='100%'></iframe>");   
                                            }else{
                                              $('#divContent').html(response.divContent);
                                            $('#grouptenant').empty(); loadTenantDraft("loadTenantDraft");
                                            $('#grouptenantlist').empty();loadgrouptenant("loadgrouptenant");
