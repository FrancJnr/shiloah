...
{    
	...
	{
            $sqlx="select a.tenanttypemasid, b.tenanttype from mas_tenant a inner join mas_tenant_type b on b.tenanttypemasid=a.tenanttypemasid where a.tenantmasid=".$_GET['tenantmasid'];
           $resulting = mysql_query($sqlx);
			if($result != null)
			{
				while($row = mysql_fetch_assoc($resulting))
				{
					//echo("<option value=".$row['tenanttypemasid'].">".$row['tenanttype']."</option>");		
	                          ...
	                          $custom = array('msg'=>"Data Grouped Successfully",'s'=>"Success","tenanttype"=>$tenanttype);
                   }
			}
