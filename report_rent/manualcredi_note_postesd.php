...
while(list($key, $val) = each($_POST['isselected'])) 
{
	if($companymasid=='1'){
	...
	//$src="../../pms_docs/creditnote/Grandways Venture Limited_331.pdf";
	 ...
	 $file = 'D:/xampp/htdocs/pms_docs/manualcreditnote/'.$companyname1.'_'.$val.'.pdf';
...
fwrite($handle, $data);
fclose($handle);
$res. = 'Copied for Signing Successfully.';
$sqlupdate = "UPDATE invoice_cr_mas SET signed = '1' WHERE  creditnoteno = $val AND companymasid= '".$companymasid."' ";
...
{
$dest = 'D:/xampp/htdocs/pms_docs/manualcreditnote/'.$companyname1.'_'.$val.'.pdf'; //pathto copy signed creditnoe to
...
fwrite($handle, $data);
fclose($handle);
$res.= 'Copied Back to system Successfully.';
$sqlupdate2 = "UPDATE invoice_cr_mas SET signed = '2' WHERE  creditnoteno = $val AND companymasid= '".$companymasid."' ";
		mysql_query($sqlupdate2);
		$res. = "Signed and Posted Successfully";
}
else
{
$res. = "The original file that you want to rename doesn't exist";
}
