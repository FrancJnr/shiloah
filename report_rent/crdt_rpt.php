...
$crdtto = date("Y-m-d",strtotime($_GET['crdtTo']));
$crdttfrom = date("Y-m-d",strtotime($_GET['crdtFrom']));
try
{    
 ?>
...
<?php
    ...
    $period =  "BETWEEN ".date("d-m-Y",  strtotime($crdttfrom))."  AND ".date("d-m-Y",  strtotime($crdtto))."";

    // where query
    $foo=false;
    
    $sqldt = "date_format(e.createddatetime,'%Y-%m-%d') BETWEEN  '$crdttfrom'  AND '$crdtto'  ";
    ...
    $tablemain .="<p class='title'><u><b>MEGA PROPERTIES GROUP CREDIT NOTE REPORT  - $period</b></p>";    
    //$message .= $tablemain;
	$bool = false;
    $sql1 ="select companymasid , companyname from mas_company WHERE companymasid = '$buildingmasid' order by companymasid  ;";
                    ...
                    for($j=0;$j<1;$j++)
                    {
                        ...
                        {                            
                            ...
                            $rent1 =0; $rentvat1=0;$sc1=0;$scvat1=0;$total1=0;
							 ...
							 //credit notes
                            $sqldet ="select a.grouptenantmasid ,c.tenantmasid,d.buildingname,creditnoteno,sum(totalvalue) as totalvaluecr,sum(round((totalvalue*14/100))) as totalvatcr
                                ,sum(totalvalue+round((totalvalue*14/100))) as totalcr
...
//                
	...
	$subject = "Credit Note Report - $period";        
...
$recipients = array();
   ...
   $recipients[$row12['email']] = $row12['staff_name']; 
    ...
    {
        echo "</br><center><h3>No Credit Notes Found for the period - $period";
   }
   ...
   $custom = array('divContent'=> "<br>".$tablemain,'s'=>'Success');
