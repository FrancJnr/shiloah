...
while($row = mysql_fetch_assoc($result))
{
	...
	if($monthdiff <= 0)
	{
	    ...
	    //if($i==1)
		 ...
		 $chkinv = "select invoiceno from invoice where
				grouptenantmasid ='$grouptenantmasid' and
				fromdate='$datefrom' and todate='$dateto' and
				rent='$rent' and sc='$sc';";
				
		$resultchkinv = mysql_query($chkinv );
		while($rowinv = mysql_fetch_assoc($resultchkinv))
	    {
		
	    $taxinvoiceno = $rowinv['invoiceno'];
		//die($taxinvoiceno);
		}
		...
		$pdf->MultiCell(0, 0, "REMARKS IF ANY:"."\n".$remarks."\n", 1, 'J', 0, 0, '' ,'', true);
                ...
                if($building_companymasid =='1')//shiloah
                {
                     /*    $pdf->SetXY(15, 272);
                        ...
                        $pdf->SetXY(180, 272);
                        $pd f->Image('../images/mm_logo.jpg', '', '', 14, 14, '', '', '', false, 300, '', false, false, 0, false, false, false);*/
