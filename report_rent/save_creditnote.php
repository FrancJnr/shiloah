...
try{
    ...
    $sqlmas="";$sqldet="";$sno="";$iid ="0";$toaddress="";$buildingmasid=0;$grouptenantmasid=0;$premise="";$crperiod="";$remarks="";$narrations="";
    ...
    foreach($_GET as $key=>$val) {
        ...
        if(($invoicedescmasid != "invoicedescmasid") and ($sno !="sno") and ($value !="value") and ($vat!="vat") and ($amount!="total"))
        {
            if($key == "companymasid"){
			   ...
			   $sqlbldname = "SELECT a.tenantmasid, b.buildingmasid, c.shortname, c.buildingname from group_tenant_mas a 
			   inner join mas_tenant b on a.tenantmasid=b.tenantmasid 
			   inner join mas_building c on c.buildingmasid=b.buildingmasid where grouptenantmasid=".$val; 
                //die($sqlbldname);
...
$taxinvoiceno=mysql_real_escape_string($taxcreditnoteno);
$rentperiod=$crperiod;
...
//$valuearrayfloat=0;$vatarrayfloat=0; changed Credit Note # '.$taxinvoiceno.' from '.$rentperiod.' for '.$buildingname.' to $narrations
   ...
   <REQUESTDATA>
    <TALLYMESSAGE xmlns:UDF="TallyUDF">
     <VOUCHER VCHTYPE="Credit Note" ACTION="Create" OBJVIEW="Accounting Voucher View">
      ...
      <STATENAME/>
      <NARRATION>Credit Note No:'.$taxinvoiceno.' -'.$narrations.'</NARRATION>
...
//End of Tally POST
    ...
    $bottom='<table cellpadding="5" border="1" width="100%">
	  ...
	  $button='<table width="100%" cellpadding="0" border="0">
                        <tr>
                            <center><td width="75%"><button type="button" id="btnSave">Post To Tally</button></td></center>
                        </tr>                                            
    </table>';    
//echo $tenantdetails.$rentdetails.$premisesdetails.$button;
