...
<html>
<head>
<title>Manual Credit Note</title>
<?php
    session_start();
...
//    }); 
    ...
    $('[id^="btnDraft"]').live('click', function() {	
	   ...
	   if ($("#oth_grouptenantmasid").css("display") == "inline")
	   {
	    ...
	    //{
	        var dataToBeSent = $("form").serialize();
	        window.open("view_manual_creditnote.php?"+dataToBeSent, "Print PDF", "width=800,height=800,toolbar:false,");
	        return false;
	    //}		  
    });
...
//	   } 	    
		...
		if($('#hid_segmentid').val()=='2')
	        ...
	        var dataToBeSent = $("form").serialize();
	        window.open("save_manual_creditnote.php?"+dataToBeSent, "Print PDF", "width=800,height=800,toolbar:false,");
	        return false;
	    }	    
    });
	
		      $('#manualcreditnteforsign').click(function(e){
		    if($(".manualcrdtsignchckbox:checked").length == 0){
			alert('No credit note selected ');
			} 
			else{
           alert('Sending to esd ');
		$('#manualcreditnteforsign').hide();

        $("#loader").show();
		   
    var form = $('#myForm2');
    var url='manualcredi_note_postesd.php';
       ...
       //var form = $(this);
            $.ajax({
			 ...
			 $.each(data.error, function(i,response){
				...
				{
				 alert(response.s);	
				 $('#manualcreditnteforsign').show();
				parent.top.$('div[name=masterdivtest]').html("<iframe  src='report_rent/rpt_manual_creditnote.php' id='the_iframe2' scrolling='yes' width='100%'></iframe>");/* 
					$.each(data.myResult, function(i,response){			
            //alert(data.msg);					
    ...
    $get_lastID = function(){	 
	 ...
	 $newRow = "<tr>\
		     ...
		     <input type='hidden' id='invoiceno_0"+$lastChar+"' name='invoiceno_0"+$lastChar+"' value='none'/>\
			...
			<tbody>
				...
				<td>
					...
					<?php loadinvoicedesc();?>
                                    ...
                                    <!--<input type="text" class='value' id="value" name="description" maxlength="255"  />-->
				</td>
<!--                                <td>
	...
	<form id="myForm2" name="myForm2" action="" method="post">
		...
		<center><button class="buttonNew" name ="manualcreditnteforsign" id="manualcreditnteforsign"> Sign to ESD </button></center>
    <?php
    $directory="../../pms_docs/manualcreditnote/";
...
$companymasid = $_SESSION['mycompanymasid'];
		...
		if(mysql_num_rows($result3) > 0)
	    {
		    while($row3 = mysql_fetch_assoc($result3))
		    {
				if($row3['signed']=='2'){
				 echo "<td>Signed</td>";
				 echo "<td><input type='checkbox' class='manualcrdtsignchckbox' name='isselectedmanualcrdt[]' value='".$creditnotenum."' disabled='true'> </td>"; 
				 }else if($row3['signed']=='1'){
				echo "<td>Pending</td>";
					echo "<td><input type='checkbox' class='manualcrdtsignchckbox' name='isselectedmanualcrdt[]' value='".$creditnotenum."'> </td>"; 
				 }else{
					echo "<td></td>"; 
					echo "<td><input type='checkbox' class='manualcrdtsignchckbox' name='isselectedmanualcrdt[]' value='".$creditnotenum."'> </td>"; 
				 }
