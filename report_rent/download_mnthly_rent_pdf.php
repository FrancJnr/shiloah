...
$sqlbuilding = 'Select buildingmasid, buildingname from mas_building order by buildingmasid;';
$resultbuilding = mysql_query($sqlbuilding);
if($resultbuilding != null)
{
    
    while($rowbuilding = mysql_fetch_assoc($resultbuilding))
    {
        $buildingmasid = '';$buildingname = '';$rect_date='';$orig_cycle='';
        
        $buildingmasid = $rowbuilding['buildingmasid'];
        $buildingname  = $rowbuilding['buildingname'];
   
$table .='<table class="custom" style="padding: 0px;font-size: 10px;font-family: Verdana, Arial, Helvetica, sans-serif;margin: 8px 0px 8px 0px;">';
