	...
	if($monthdiff <= 0)
	{
		...
		if($resultchk)
		{
			...
			$insert = "insert into invoice (grouptenantmasid,invoiceno,fromdate,todate,rent,sc,createdby,createddatetime) values
                        ($grouptenantmasid,'$taxinvoiceno','$datefrom','$dateto',$rent,$sc,'$createdby','$createddatetime')";
			mysql_query($insert);		
...
$dates=str_replace('-', 'at', date("j M Y - h:i", strtotime($dateno)));
	   ...
	   $filename = preg_replace('/[^A-Za-z0-9() -]/', '', $taxinvoiceno);
	   $pdf->lastPage();
           // Send to get PDF folder on esd
		   if($companymasid=='1'){ // If shiloah check shiloah esd path
	$pdf->Output("C:/GetPdfs/".$filename.".pdf","F");
	$filename1 = "C:/GetPDFs/Signed/".$filename."*"; // Search signed file.
}
sleep(20); //wait 15 secods to allow machine to sign



foreach (glob($filename1) as $filefound) {
	  if (file_exists($filefound)) // if signed file exixts
{
$dest = 'D:/xampp/htdocs/pms_docs/invoice/'.$filename.'.pdf'; //pathto copy signed creditnoe to
...
fwrite($handle, $data);
fclose($handle);
echo 'Copied back to PMS Successfully.';
}
else
{
