...
<?php
		...
		if($_SERVER['REQUEST_METHOD'] == "POST")  
		{
			...
			$update ="update rpt_simple_agreement set rowcontent = $txt where grouptenantmasid =$post_tenantmasid";
			mysql_query($update);
			//echo $update;
...
$(document).ready(function() {	
     var options = {
        ...
        loadSimpleTenant("loadSimpleTenant");
        function loadSimpleTenant(itemtype)
	{		
		...
		$.getJSON(url,function(data){
			...
			if(response.s == "Success")
			{
			    ...
			    $('#grouptenantmasid').append( new Option("-----Select Simple Tenant-----","",true,false) );
	...
	if($("#grouptenantmasid option:selected").val()== "")
	{
		...
		var url="save_simple_agreement.php";
	...
	$('#btnPrint').click(function(){
		...
		var url="save_print_simple_agreement.php";
		var dataToBeSent = $("form").serialize();
		window.open("save_print_simple_agreement.php?"+dataToBeSent,  "windowOpenTab", "width=1800,height=800,scrollbars=yes,resizable=yes,toolbars:yes");
                return false;	
...
<table id="usertbl" class="table2" width="80%">
	...
	<tr>
		...
		<td>
			<select id="grouptenantmasid" name="grouptenantmasid" style='width:525px;'>
				<option value="" selected>----Select Simple Tenant----</option>
			</select>
		</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td>
			...
			<button type="button" id="btnPrint">Save & Print</button>
		</td>
	</tr>
	</tbody>
</table>
