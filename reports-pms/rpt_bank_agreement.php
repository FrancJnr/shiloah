...
<?php
		...
		if($_SERVER['REQUEST_METHOD'] == "POST")  
		{
			...
			$update ="update rpt_bank_agreement set rowcontent = $txt where grouptenantmasid =$post_tenantmasid";
			mysql_query($update);
			//echo $update;
...
$(document).ready(function() {	
     var options = {
        ...
        loadBankTenant("loadBankTenant");
        function loadBankTenant(itemtype)
	{		
		...
		$.getJSON(url,function(data){
			...
			if(response.s == "Success")
			{
			    ...
			    $('#grouptenantmasid').append( new Option("-----Select Bank Tenant-----","",true,false) );
	...
	if($("#grouptenantmasid option:selected").val()== "")
	{
		...
		var url="save_bank_agreement.php";
	...
	$('#btnPrint').click(function(){
		...
		var dataToBeSent = $("form").serialize();
		window.open("save_print_bank_agreement.php?"+dataToBeSent,  "windowOpenTab", "width=1800,height=800,scrollbars=yes,resizable=yes,toolbars:yes");
                return false;	
	...
	<tr>
		...
		<td>
			<select id="grouptenantmasid" name="grouptenantmasid" style='width:525px;'>
				<option value="" selected>----Select Bank Tenant----</option>
			</select>
		</td>
	</tr>
	<tr>
...
<?php
$directory="../../pms_docs/banktemplate/";
