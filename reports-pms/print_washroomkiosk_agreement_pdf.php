...
try
{
$sql = "SELECT * FROM draft_document1 WHERE section='washroom-agreement' AND grouptenantmasid=".$grouptenantmasidz." AND tenantmasid=".$tenantmasidz."  ORDER BY draftid DESC LIMIT 1";
...
//    
    ...
    $sql = "select * from rpt_washroomkiosk_agreement where grouptenantmasid =$groupmasid";
    $result = mysql_query($sql);
    $rowcount = mysql_num_rows($result);
    
    if($rowcount <=0)
    {
        $pdf->Output("../../pms_docs/washroom/".$filename.".pdf","F");
        $insert = "insert into rpt_washroomkiosk_agreement (grouptenantmasid,createdby,createddatetime) values ($groupmasid,'$createdby','$createddatetime')";
        mysql_query($insert);
    }
