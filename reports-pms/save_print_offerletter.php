...
<script type="text/javascript" language="javascript">
  $(document).ready(function() {
	...
	{
          var url = "print_offerletter_pdf.php?grouptenantmasid="+<?php echo $_GET['grouptenantmasid'];?>+"&tenantmasid="+<?php echo $tenamentvalue;?>;
...
//			var url="save_draft_document.php?action=Save&section=offerletter";
...
//			});
    ...
    function ajaxPOSTTest() {
      //  alert("haha");
        ...
        var url = "save_draft_document.php?action=Save&section=offerletter&grouptenantmasid="+<?php echo $_GET['grouptenantmasid'];?>+"&tenantmasid="+<?php echo $tenamentvalue;?>;
        var params =$("form").serialize();
        ajaxPOSTTestRequest.open("POST", url, true);
        ajaxPOSTTestRequest.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        ajaxPOSTTestRequest.send(params);
...
//    }
    ...
    $p7 ='<b>'.$x.'. RENT:</b>
            ...
            <ol>';
	$x++;    
    $p8 ='<b>'.$x.'. FORM OF LEASE:</b><ol align="justify" style="line-height: 1.5px;letter-spacing:+0.100mm;font-stretch:100%;list-style-type: none;"><li>The Lease will be in the landlords Standard Form of Lease for <strong> '.strtoupper($companyname).'</strong> as shall be prepared by the Landlords Advocates which shall be deemed to have been accepted by the Tenant upon acceptance of the terms of this letter.</li><ol>';
	$p9="";
	$p10="";
//    $pdf->SetMargins(PDF_MARGIN_LEFT, PDF_MARGIN_TOP, PDF_MARGIN_RIGHT);
...
//    $pdf->ln(3);
    ...
    $p9 ='<b>'.$x.'. SERVICE CHARGE:</b>
            <ol align="justify" style="line-height: 1.5px;letter-spacing:+0.100mm;font-stretch:100%;list-style-type: none;"><li>You will also pay Actual Service Charge determined in the manner stated in the Lease to reimburse the Landlords for a fair proportion of the operation expenses of the building that shall include but not be limited to the following: -
		</li><li><ol align="justify" style="line-height: 1.8px;letter-spacing:+0.100mm;font-stretch:100%;list-style-type: lower-roman;">
...
//	$pdf->ln(3);
    ...
    $x++; 
    $p11 ='<br><b>'.$x.'. STATUTORY TAXES:</b><ol align="justify" style="line-height: 1.5px;letter-spacing:+0.100mm;font-stretch:100%;list-style-type: none;">
                <li>
...
//    $pdf->ln(3);
    ...
    $x++; 
    $p15 ='<br><b>'.$x.'. LAYOUT AND PARTITIONING:</b>
            <ol align="justify" style="line-height: 1.5px;letter-spacing:+0.100mm;font-stretch:100%;list-style-type: none;">
                <li>
...
//    $pdf->ln(3);
    ...
    $p21 ='<br><b>'.$x.'. ACCEPTANCE OF OFFER LETTER:</b>
		...
		<li>
            ...
            <ol>';
             $x++; 
    $p21 .='<br><b>'.$x.'. MODE OF PAYMENT:</b><br>
...
//    $pdf->ln(3);
    ...
    $p22 ='<br>PLEASE NOTE:</b>
     ...
     $px="We confirm that we wish to proceed on the basis of the terms set out in this letter";//, true, false, true, false, '');
//    $pdf->ln(1);
     if($renewalfromid >0){
	$prevdeposit =0;
	...
	if($prevdeposit >0)
	{
		$px1="Carry forward from previous deposit amount Kshs ".number_format($prevdeposit, 0, '.', ',')."/= .";//, true, false, true, false, '');
		//$pdf->ln(1);
        $px2="The new deposit amount Kshs ".number_format($scdepo+$rentdepo, 0, '.', ',')."/= .";//, true, false, true, false, '');
		//$pdf->ln(1);
        $px3="Balance payable Kshs ".number_format($totalDeposit-$prevdeposit, 0, '.', ',')."/= .";//;, true, false, true, false, '');
		//$pdf->ln(1);
	}
       }
	if($totalDeposit !=0)
	{
		$px4="We enclose our cheque for the sum of <strong>KShs.".number_format($totalDeposit-$prevdeposit, 0, '.', ',')."/= </strong> as set out herein below.";//, true, false, true, false, '');
		//$pdf->ln(2);
		$px5=$tenantDeposit;//, true, false, true, false, '');
		//$pdf->ln(2);		
	}
    $p23 ='
...
//    $filename = clean($filename);
   ...
   //running// $pdf->Output("../../pms_docs/offerletters/".$filename.".pdf","F");
...
$sql = "SELECT * FROM draft_document1 WHERE section='offerletter' AND grouptenantmasid=".$_GET['grouptenantmasid']." AND tenantmasid=".$tenamentvalue."  ORDER BY draftid DESC LIMIT 1";
$sql2="SELECT * FROM draft_document2 WHERE section='offerletter' AND grouptenantmasid=".$_GET['grouptenantmasid']." AND tenantmasid=".$tenamentvalue."  ORDER BY draftid DESC LIMIT 1";
...
if( !empty($rowprint) && !empty($row2print)  )
{
//    echo $pa;
//    echo '<br>';
//    echo $pb;
    ...
    $p23 =$row2print['p23'];
  //  echo $pa;
  echo '<form><textarea name="pa" class="jqte-test">'.preg_replace('/\s+/',' ',$pa).'</textarea>
   <textarea  name="pb" class="jqte-test">'.preg_replace('/\s+/',' ',$pb).'</textarea>
   <textarea  name="pc" class="jqte-test">'.preg_replace('/\s+/',' ',$pc).'</textarea>
   <textarea  name="pd" class="jqte-test">'.preg_replace('/\s+/',' ',$pd).'</textarea>
   <textarea  name="p0" class="jqte-test">'.preg_replace('/\s+/',' ',$p0).'</textarea>
   <textarea  name="p1" class="jqte-test">'.preg_replace('/\s+/',' ',$p1).'</textarea>
   <textarea  name="p2" class="jqte-test">'.preg_replace('/\s+/',' ',$p2).'</textarea>
   <textarea  name="p3" class="jqte-test">'.preg_replace('/\s+/',' ',$p3).'</textarea>
   <textarea  name="p4" class="jqte-test">'.preg_replace('/\s+/',' ',$p4).'</textarea>
   <textarea  name="p4a" class="jqte-test">'.preg_replace('/\s+/',' ',$p4a).'</textarea>
   <textarea  name="p5" class="jqte-test">'.preg_replace('/\s+/',' ',$p5).'</textarea>
   <textarea  name="p6" class="jqte-test">'.preg_replace('/\s+/',' ',$p6).'</textarea>
   <textarea  name="p7" class="jqte-test">'.preg_replace('/\s+/',' ',$p7).'</textarea>
   <textarea  name="p8" class="jqte-test">'.preg_replace('/\s+/',' ',$p8).'</textarea>
   <textarea  name="p9" class="jqte-test">'.preg_replace('/\s+/',' ',$p9).'</textarea>
   <textarea  name="p10" class="jqte-test">'.preg_replace('/\s+/',' ',$p10).'</textarea>
   <textarea  name="p11" class="jqte-test">'.preg_replace('/\s+/',' ',$p11).'</textarea>
   <textarea  name="p12" class="jqte-test">'.preg_replace('/\s+/',' ',$p12).'</textarea>
   <textarea  name="p13" class="jqte-test">'.preg_replace('/\s+/',' ',$p13).'</textarea>
   <textarea  name="p14" class="jqte-test">'.preg_replace('/\s+/',' ',$p14).'</textarea>
   <textarea  name="p15" class="jqte-test">'.preg_replace('/\s+/',' ',$p15).'</textarea>
   <textarea  name="p16" class="jqte-test">'.preg_replace('/\s+/',' ',$p16).'</textarea>
   <textarea  name="p17" class="jqte-test">'.preg_replace('/\s+/',' ',$p17).'</textarea>
   <textarea  name="p18" class="jqte-test">'.preg_replace('/\s+/',' ',$p18).'</textarea>
   <textarea  name="p19" class="jqte-test">'.preg_replace('/\s+/',' ',$p19).'</textarea>
   <textarea  name="p20" class="jqte-test">'.preg_replace('/\s+/',' ',$p20).'</textarea>
   <textarea  name="p21" class="jqte-test">'.preg_replace('/\s+/',' ',$p21).'</textarea>
   <textarea  name="p22" class="jqte-test">'.preg_replace('/\s+/',' ',$p22).'</textarea>
   
   
   <textarea  class="jqte-test">'.preg_replace('/\s+/',' ',$px).'</textarea>
   <textarea  class="jqte-test">'.preg_replace('/\s+/',' ',$px1).'</textarea>
   <textarea  class="jqte-test">'.preg_replace('/\s+/',' ',$px2).'</textarea>
   <textarea  class="jqte-test">'.preg_replace('/\s+/',' ',$px3).'</textarea>
   <textarea  class="jqte-test">'.preg_replace('/\s+/',' ',$px4).'</textarea>
   <textarea   class="jqte-test">'.preg_replace('/\s+/',' ',$px5).'</textarea>
   
   <textarea  name="p23" class="jqte-test">'.preg_replace('/\s+/',' ',$p23).'</textarea>
</form>';
}else{
//echo "Empty";
echo '<form><textarea name="pa" class="jqte-test">'.preg_replace('/\s+/',' ',$pa).'</textarea>
...
<!--<form>
   ...
   <textarea  name="p11" class="jqte-test"><?php echo $p11; ?></textarea>
   <textarea  name="p12" class="jqte-test"><?php echo $p12; ?></textarea>
   <textarea  name="p13" class="jqte-test"><?php echo $p13; ?></textarea>
   <textarea  name="p14" class="jqte-test"><?php echo $p14; ?></textarea>
   <textarea  name="p15" class="jqte-test"><?php echo $p15; ?></textarea>
   <textarea  name="p16" class="jqte-test"><?php echo $p16; ?></textarea>
   <textarea  name="p17" class="jqte-test"><?php echo $p17; ?></textarea>
   <textarea  name="p18" class="jqte-test"><?php echo $p18; ?></textarea>
   <textarea  name="p19" class="jqte-test"><?php echo $p19; ?></textarea>
   <textarea  name="p20" class="jqte-test"><?php echo $p20; ?></textarea>
   <textarea  name="p21" class="jqte-test"><?php echo $p21; ?></textarea>
   <textarea  name="p22" class="jqte-test"><?php echo $p22; ?></textarea>
    <textarea  name="p23" class="jqte-test"><?php echo $p23; ?></textarea>
   
   <textarea  class="jqte-test"><?php echo $px; ?></textarea>
   <textarea  class="jqte-test"><?php echo $px1; ?></textarea>
   <textarea  class="jqte-test"><?php echo $px2; ?></textarea>
   <textarea  class="jqte-test"><?php echo $px3; ?></textarea>
   <textarea  class="jqte-test"><?php echo $px4; ?></textarea>
   <textarea   class="jqte-test"><?php echo $px5; ?></textarea>
...
<span class="jqte-test"><b>My contents are from <u><span style="color:rgb(0, 148, 133);">SPAN</span></u></b></span>-->
