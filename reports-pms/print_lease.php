...
//                    "rule link image unlink | cut copy paste pastetext | print source"
	...
	$('#btnView').click(function(){
		...
		if(r==true)
		{
			...
			window.open("save_print_lease.php?"+dataToBeSent, "windowOpenTab", "width=1800,height=800,scrollbars=yes,resizable=yes,toolbars:yes");
			return false;				
		}
		...
		//		else
		//		{
		//			$('#cc').html(response.divContent);
		//		}
		//	});
		//});	
	});	
	$('[id^="btnGuarantee"]').live('click', function() {
		if($("#grouptenantmasid option:selected").val()== "")
		{
			alert("Please select tenant");
			$('#grouptenantmasid').focus();
		...
		if($('#grouptenant_contact').val()=='')
		{
			alert("Please enter Director Name");
			$('#grouptenant_contact').focus();
			return false;
		}
		var r = confirm("can you confirm this?");
		if(r==true)
		{
			var dataToBeSent = $("form").serialize();
			window.open("save_print_lease_guarantee.php?"+dataToBeSent, "Print PDF", "width=800,height=800,toolbar:false,");
			return false;				
		}
	});
	...
	$('[id^="grouptenantmasid"]').live('change', function() {		
		...
		var url="load_report.php?item=grouptenant_contact&itemval="+$(this).val()+"&buildingshortname="+temp[0];	
		$.getJSON(url,function(data){
			$.each(data.error, function(i,response){
				$.each(data.myResult, function(i,response){
					$('#grouptenant_contact').val(response.cpname);
					$('#grouptenant_contact_desig').html(response.cptype);
				});
			});		
		});
...
<div id="container">
	<br>
<h1>Lease and Guarantee Document</h1>
<br>
<table id="usertbl" class="table1">
	<thead>
		<tr>
			<th id="tblheader" align="left" colspan="2">
				Generate Lease & Guarantee Document 
			</th>
		</tr>
	</thead>
	<tbody>
	<tr>
		<td>			
			<select id="grouptenantmasid" name="grouptenantmasid" style='width:525px;'>
				...
				Director Name &nbsp;&nbsp;<input type='text' id='grouptenant_contact' name='grouptenant_contact' value='' />
				<span class='span_cont' id='grouptenant_contact_desig'></span>				
		</td>
		
	</tr>
	<tr>		
		<td colspan='2'>
			<button type="button" id="btnView">Print Lease</button>
			<button type="button" id="btnGuarantee">Print Guarantee</button>
		</td>
	</tr>	
	</tbody>
</table>
