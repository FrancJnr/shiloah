...
try
{
$sql = "SELECT * FROM draft_document1 WHERE section='simple-agreement' AND grouptenantmasid=".$grouptenantmasidz." AND tenantmasid=".$tenantmasidz."  ORDER BY draftid DESC LIMIT 1";
...
foreach ($_GET as $k=>$v) {	
    ...
    $pdf->setHtmlHeader("TENANT NAME:<b> ".$tradingname."</b>");
    ...
    if($rowcount <=0)
    {
        $pdf->Output("../../pms_docs/simple/".$filename.".pdf","F");
        $insert = "insert into rpt_simple_agreement (grouptenantmasid,createdby,createddatetime) values ($groupmasid,'$createdby','$createddatetime')";
        mysql_query($insert);
    }
