...
$table .="<tr><td colspan='18'><b>RENT FOR THE PERIOD : ".strtoupper(date('M Y', strtotime($_GET['fromdt']))." - ".date('M Y', strtotime($_GET['todt'])))."</b></td></tr>";
...
for($i=0;$i<=$months;$i++)
{
    ...
    $table .="<th class='rowth' align='center'>$d2</th>";    
}
$table .="<th>Total</th>";
...
$sqlm=" select TIMESTAMPDIFF(MONTH, rk.olfd, rk.oltd)+1  as monthsdiff from (
            select min(ol.fd) as olfd,max(ol.td) as oltd from (
                select max(date_format(e.fromdate,'%Y-%m-%d')) as fd,max(date_format(e.todate,'%Y-%m-%d')) as td
                        ...
                        where c.buildingmasid ='$buildingmasid'  and e.rent>0 and
...
$n =1;$cnt=0;$invno="";$md="";$invsc="";$grandlinetotal=0;
    ...
    {        
        while($row1 = mysql_fetch_assoc($result1)) // no of tenants available for the building
        {
            ...
            $mnthschrgd1=0;$mnthschrgd=0;$td1="";$td2="";$td="";$size="";$totalsqrft=0;$rent=0;$linetotal=0;$s=0;
            ...
            {                                            
                ...
                $sql2 ="select a.rent, a.sc,a.fromdate,a.todate,a.grouptenantmasid,c.tenantmasid,c.renewalfromid,
                        ...
                        case lower(f.shortdesc)
                                when 'mnthly' then round(a.rent/1)
                                when 'qtrly' then round(a.rent/3)                    
                                when 'half' then round(a.rent/6)
                                when 'yearly' then round(a.rent/12)
                        end as 'mnthlyrentold',
                        case lower(f.shortdesc)
                        ...
                        round(a.rent/@m) as mnthlyrentold,a.rent as mnthlyrent,
                        ...
                        inner join mas_building e on e.buildingmasid = d.buildingmasid
                        inner join mas_age f on f.agemasid = c.agemasidrc
                        where a.rent>0 and a.grouptenantmasid = '$grouptenantmasid' and date_format(a.fromdate,'%Y-%m') = '$d2'
                        union
                ...
                if($result2 !=null)
                {
                    ...
                    if($rcount ==0)
                    {
                        if($f<=$months)
                        {                            
                            $cls = "row".$s;$renttotal1=0;
                            $rent1=0;$sc1=0;
                            $sql3 ="select @s:=TIMESTAMPDIFF(MONTH, fromdate, todate)+1 as md, date_format(fromdate,'%d') as fd, round(rent/@s) as mnthlyrentprev,round(sc/@s) as mnthlyscprev
                                    from advance_rent
                                    where grouptenantmasid = '$grouptenantmasid' and '$d2' between date_format(fromdate,'%Y-%m') and  date_format(todate,'%Y-%m')
                                    union
                                    select @s:=TIMESTAMPDIFF(MONTH, fromdate, todate)+1 as md, date_format(fromdate,'%d') as fd,round(rent/@s) as mnthlyrentprev,round(sc/@s) as mnthlyscprev
                                    from invoice
                            ...
                            if($result3 !=null)
                            {
                                ...
                                {                                    
                                    ...
                                    {
                                        $rent1 = $row3['mnthlyrentprev'];
                                        $sc1 = $row3['mnthlyscprev'];
                                        $renttotal1=$rent1+$sc1;
                                        if ($rent1>0)
                                        $mnthschrgd ++;                                    
                                        $linetotal +=$rent1;    
                                    }
                            ...
                            $td .="<td class='row".$s."' align='right'>".number_format($rent1, 0, '.', ',')."-".number_format($sc1, 0, '.', ',')."</td>";
                            //$td .="<td class='row".$s."' align='right'>".number_format($renttotal1, 0, '.', ',')."</td>";                            
                    ...
                    {                                                
                        ...
                        {                                                                                                               
                            ...
                            $invsc = $row2['mnthlyrent'];
                            $df1 = $row2['fromdate'];
                            $dtt = $row2['invfrom']."--".$row2['invto'];;
                            $md=$row2['monthsdiff'];
                            $sc = $row2['mnthlysc'];                            
                            $rent = $row2['mnthlyrent'];                            
                            $rcount = mysql_num_rows($result2);
                            $rent = round($rent/$md);
                            $sc = round($sc/$md);
                            //if($grouptenantmasid == '252')
                            //$td .="<td class='row".$s."' align='right'>$rent</td>";
                            ...
                            {                                                                                                
                                ...
                                {                                                                       
                                    ...
                                    $linetotal +=$rent;
                                    $renttotal = $rent+$sc;
                                    $td .="<td class='row".$s."' align='right'>".number_format($rent, 0, '.', ',')." - ".number_format($sc, 0, '.', ',')."</td>";
                                    //$td .="<td class='row".$s."' align='right'>".number_format($renttotal, 0, '.', ',')."</td>";
