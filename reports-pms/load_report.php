...
else if($load == "loadTenantFinalized")
{
    ...
    (select tenantmasid from trans_offerletter where editpermission=0) order by leasename asc";
}
...
{   
    ...
    $sql= "select b.leasename,a.tenantmasid,b.tenantcode,c.shopcode,c.size from group_tenant_det a
            ...
            inner join mas_shop c on  c.shopmasid = b.shopmasid
            where a.grouptenantmasid = $id and b.active='1' order by leasename asc"; 
...
{   
    ...
    $table .="</br>Deposit:</br>";
    $sql = "select a.rentdeposit,a.scdeposit,a.stampduty,a.leegalfees,a.createdby,date_format(a.createddatetime,'%d-%m-%Y') as createdon from rec_trans_offerletter_deposit a
            inner join rec_trans_offerletter b on b.offerlettermasid = a.offerlettermasid
            inner join mas_tenant c on c.tenantmasid = b.tenantmasid
            inner join group_tenant_det d on d.tenantmasid = c.tenantmasid
            where d.grouptenantmasid =$id;";
...
else if($load == "loadleaseforsurrender")
{
   //for auto complete
    ...
    $sql= " select leasename,tenantmasid,tradingname,tenantcode,renewalfromid,shopcode,size,grouptenantmasid,buildingname,active,shopoccupied
            ...
            where leasename like '%".$term."%' or tradingname like '%".$term."%';";
   $query=mysql_query($sql);
   $json=array();
   $renewalfromid =0;$active=0;$shopoccupied=0;
   while($row=mysql_fetch_array($query))
...
else if($load == "loadSimpleTenant")
{
    $sql= "select c.doc,c.leasename,c.tradingname,c.renewalfromid,d.shopcode, a.grouptenantmasid from group_tenant_mas a 
            inner join mas_tenant c on c.tenantmasid = a.tenantmasid
            inner join mas_shop d on  d.shopmasid = c.shopmasid 
            where c.companymasid=$companymasid and c.active ='1' and c.tenanttypemasid='2' and c.leasename not in (select c.leasename from rpt_surrender_lease a
...
{   
    ...
    $sql ="select a.grouptenantmasid , b.leasename,b.tradingname,b.tenantcode,j.buildingname,
      ...
      where a.grouptenantmasid =$grouptenantmasid and i.documentname='1' and c.companymasid=$companymasid order by b.leasename asc;";
