...
foreach ($_GET as $k=>$v) {	
	...
	{		
		...
		{		
			...
			//create tenant view
			$viewSql1 =  "create view view_offerletter_tenant as ".$sqlv1;
			$result1 = mysql_query($viewSql1);
			...
			$viewSql2 =  "create view view_tenant_cp as ".$sqlv2;
			$result2 = mysql_query($viewSql2);
			
			// create tenant rent tbl view
			$viewSql3 = "create view view_offerletter_sctype as select sctype,basicscval from \n"
			    . "trans_offerletter a\n"			    
			    . "WHERE a.tenantmasid =".$tenantmasid;
			$result3 = mysql_query($viewSql3);
			
			// create tenant rent tbl view
			$viewSql4 = "create view view_offerletter_rent as select b.*,DATE_FORMAT( b.fromdate, '%d-%m-%Y' ) as 'rent_fromdate' ,DATE_FORMAT( b.todate, '%d-%m-%Y' ) as 'rent_todate' from \n"
			    . "trans_offerletter a\n"
			    . "inner join trans_offerletter_rent b on b.offerlettermasid = a.offerlettermasid\n"
			    . "WHERE a.tenantmasid =".$tenantmasid;
			$result4 = mysql_query($viewSql4);
			
			// create tenant scrvice charge tbl view
			$viewSql5= "create view view_offerletter_sc as select b.*, DATE_FORMAT( b.fromdate, '%d-%m-%Y' ) as 'sc_fromdate' ,DATE_FORMAT( b.todate, '%d-%m-%Y' ) as 'sc_todate' from \n"
			    . "trans_offerletter a\n"
			    . "inner join trans_offerletter_sc b on b.offerlettermasid = a.offerlettermasid\n"
			    . "WHERE a.tenantmasid =".$tenantmasid;
			$result5 = mysql_query($viewSql5);
			
			// create tenant deposit tbl view
			$viewSql6 = "create view view_offerletter_deposit as select b.* from \n"
			    . "trans_offerletter a\n"
			    . "inner join trans_offerletter_deposit b on b.offerlettermasid = a.offerlettermasid\n"
			    . "WHERE a.tenantmasid =".$tenantmasid;
			$result6 = mysql_query($viewSql6);
...
//                                                   $rentcycle=  strtolower($rk['rentcycle']);
			...
			$tenantRent .='<table cellpadding="1" cellspacing="0" border="1" width="100%"><tr align="center" style="font-weight: bold">
                                        <th>FROM</th>
                                        <th>TO</th>
                                        <th>AMOUNT</th>
                                        <th>INTERVAL</th>
			...
			$tenantSc .='<table cellpadding="1" cellspacing="0" border="1" width="100%"><tr align="center" style="font-weight: bold">
                                        <th>FROM</th>
                                        <th>TO</th>
                                        <th>AMOUNT</th>
                                        <th>INTERVAL</th>
			...
			$tenantSc .='</table><br>';
			$leasedetails .='<table cellpadding="3" align="center" cellspacing="0" border="1" >
			<tr align="justify">
			<td style="font-weight: bold;width:30%;padding-left:10px">Date of Lease</td>
			<td style="padding-left:10px;width:70%">'.date('jS F Y').'</td>
			</tr>
             <tr align="justify">
			 <td style="font-weight: bold;padding-left:10px">Lessor</td>
			 <td style="padding-left:10px"> <b>'.strtoupper($companyname).' </b>,  a limited liability company incorporated in the Republic of Kenya and of Post Office Box Number 2501-40100, Kisumu. </td>
			 </tr>
			 <tr align="justify">
			 <td style="font-weight: bold;padding-left:10px">Lessee</td>
			 <td style="padding-left:10px"><strong>'.strtoupper($tenantname).'</strong> '.$leaseclass.' a limited liability company registered under the laws of the Republic of Kenya and of '.$tenantshortaddress.' .</td>
			 </tr> 
			 <tr align="justify">
			 <td style="font-weight: bold;padding-left:10px">Demised <br> Premises</td>
			 <td style="padding-left:10px">The <strong> '.rtrim($shopid,',').' at '.strtoupper($buildingname).'; </strong> the premises comprise a total of  approximately <strong>'.$sizetotal.' </strong> square feet and are more particularly delineated  in  red on  the plan annexed this lease (hereinafter called '.'the demised premises'.')</td>
			 </tr>
			 <tr align="justify"><td style="font-weight: bold;padding-left:10px">Term</td><td style="padding-left:10px"><strong>'.$leaseterm.'</strong>  from  <strong>'.date('jS F Y', strtotime($doc)).'</strong></td></tr>
			  <tr align="justify"><td  style="font-weight: bold;padding-left:10px">Rent/Stand <br><br> Premium</td><td>'.$tenantRent.'</td></tr>
			  <tr align="justify"><td style="font-weight: bold;padding-left:10px"> Service Charge </td><td>'.$tenantSc.'</td></tr>
			   <tr align="justify"><td style="font-weight: bold;padding-left:10px">Cadastral Plan of the <br> Demised Premises (where <br> applicable)</td><td style="padding-left:10px"></td></tr>
			   <tr align="justify"><td style="font-weight: bold;padding-left:10px">Sectional Plan(s) of the <br> Demised Premises (where <br> applicable)</td><td style="padding-left:10px"></td></tr>
...
//    $pdf->Cell(70);
//    $pdf->Cell(40,10,'PRESENTATION BOOK No:');
//    $pdf->Cell(25);
...
//    $pdf->ln(2);
    ...
    $txt ="<strong>THIS LEASE </strong> witnesses as follows: <br>";    
    $pc=$txt;
...
//    $pdf->writeHTML($tenantRent, true, false, true, false, '');
   ...
   // $html = "<b><u>For the Space of $sizetotal Square Feet:-</u></b>";
    $pd='The Lessor as legal owner of the above noted Title <strong>HEREBY LEASES </strong> to the Lessee the Demised Premises for the Term subject to the payment of the Rent and subject to the conditions set out in this Lease:
';
...
//    $pdf->SetFont('dejavusans','',8.8);
    $html ='<ol align="justify" style="letter-spacing:+0.100mm;font-stretch:100%;list-style-type: lower-alpha;">
            ...
            <li>The  Lessee shall deposit with the Lessor a <b> deposit of KShs.'.number_format($totalDeposit, 0, '.', ',').'/= </b>to be held by  the Lessor during the period of  the lease free of  
                interest as security for  the payment of  rent ,service charge, undertaking repairs that may have to be carried out within the demise
                premises and the performance of all  the Lessee'."'".'s obligations and covenants herein contained. If the rent or  any other monies payable 
                by  the Lessee under this lease shall not be paid on  its due date the Lessee shall pay to the Lessor penalty for  late payment on  the
				said monies to be calculated at 2%   per month until payment thereof in  full  <u> SUBJECT  HOWEVER </u> to the  following terms and conditions and covenants:-
                <br>
            </li>
            <li>To  pay to the Lessor in  addition to the said rent by way of  reimbursement to the Lessor of  the operating expenses of  
			<b>'.strtoupper($buildingname).'</b> a SERVICE CHARGE to be calculated as a percentage which the floor space of the demised premises
                ...
                <strong>'.$scterm.' </strong> and as tabulated above and shall be used to pay the following:- :-<br><br>                
		...
		Lessee may require on the demised premises and all rentals and other charges payable in respect of
                ...
                <ol style="list-style-type: lower-roman">
                    <li>To pay to the appropriate electricity authority any sums or charges payable in respect of the installation
                        ...
                        Lessor indemnified in respect thereof OR;<br>
                    </li>
...
//    $pdf->ln(2);
 ...
 //   $pdf->AddPage();
    $txt1 ='<table width="100%" border="0"> 
	<tr> <td>4)    <b>MODE OF PAYMENT</b></td></tr>
	<tr> <td>All payments to be made, strictly, either through bank deposit or Cheque / Banker’s cheque only.  
	In case of Bank deposit, it will be on the company account only.  Once the transfer is done, 
	the deposit slip to be emailed to confirm the payment and issue necessary receipt.<b>NO CASH PAYMENT ACCEPTED.</b></td></tr>
	</table>';
...
//    $pdf->SetFont('dejavusans','',9);
    ...
    if ($tradingname == "")
    {
        ...
        $html_1 ='<table width="100%" border="0">
		 <tr><td>The lessee hereby accepts this lease subject to the above conditions, restrictions and stipulations.</td></tr>

        <tr>
            <td>IN WITNESS WHEREFORE the Lessor and the Lessee have caused their Common Seal to be affixed on
       ...
       <tr>
        ...
        <tr>
         ...
         <tr>
            <td style="height:10px;">SEALED with the Common Seal of </td>
            <td>]</td>
        </tr>
        <tr>
            <td style="height:10px;">The Lessee: <b>'.$tenantname.'</b></td>
            <td>]</td>
        </tr>
         <tr>
            <td style="height:10px;">In the presence of: -</td>
            <td>]</td>
        </tr>
       <tr>
        ...
        <tr>
            <td colspan="2" style="height:10px;">ADVOCATE</td>            
        </tr>
         <tr>
        ...
        <tr>
            <td colspan="2" align="center" style="height:30px;"><br><br><b>______________________________________<br><i>
    ...
    else
    {
		...
		$html ='<table width="100%" border="0">
         <tr><td colspan="2">The lessee hereby accepts this lease subject to the above conditions, restrictions and stipulations.</td></tr>
		 <tr>
            <td colspan="2" style="height:20px;" align="justify">IN WITNESS WHEREFORE the Lessee and the Lessor have caused their Common Seal to be affixed on
        ...
        <tr>
            <td style="height:10px;">The Lessor: <b>'.$companyname.'</b></td>
            <td>]</td>
        </tr>
         <tr>
            <td style="height:10px;">In the presence of: -</td>
       ...
       <tr>
        ...
        <tr>
            <td style="height:25px;">_____________________________<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            &nbsp;&nbsp;
            DIRECTOR</td>
            <td>]&nbsp;&nbsp;&nbsp;&nbsp;__________________________<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            &nbsp;&nbsp;&nbsp;
        ...
        <tr>
            <td colspan="2" style="height:20px;">ADVOCATE</td>            
        </tr>
        <tr>
            <td colspan="2" align="center" style="height:25px;"><b><u>CERTIFICATE</u></b></td>            
        </tr>
        ...
        <tr>
            <td style="height:40px;"><b>'.$tenantname.'</b><br></td>
            <td>]</td>
        </tr>         
        <tr>
            <td style="height:30px;">_____________________________<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	...
	if($pledged == 1)
	{
           $html ='<table width="100%" border="0">
			    ...
			    way our rights, interests and preference therein.
                    ...
                    <tr>
					<td  valign="left" style="height:50px;"><strong>SIGNED BY:</strong><br>
					The duly constituted Attorney of <br><strong>  '.$pledgedinbank.' </strong>,

					</td>
		           </tr>
		   		
                    <tr>
                            <td  valign="left" style="height:50px;">Under & by virtue of Power of the Attorney:<br>Registered at the Lands Registry at Nairobi as</td>
                    </tr>
                    
                    <tr>
                            <td  valign="left" style="height:50px;"><strong>PA_______________________</strong></td><td style="height:50px;">Signature_______________________</td><br>In the presence of: -
                    </tr>
                     <tr>
                            <td  valign="left" style="height:50px;"><strong>PA_______________________</strong></td><td style="height:50px;">Signature_______________________</td>
                    </tr>
                   
                     <tr>
                            <td  valign="left" style="height:50px;">______________________________<br><strong>Advocate</strong></td>
                    </tr>
                  

                     <tr>
                            <td  valign="middle" style="height:50px;"><strong> <u>CERTIFICATE</u><br></strong></td>
                    </tr>
                    <tr>
                            <td  valign="left" style="height:50px;">I, __________________________________Advocate of the High Court, certify that the above Attorneys of Cooperative Bank of Kenya Limited appeared before me on this _______ day of __________________'.convert_number(date("Y")).' and being  known /shown to me acknowledged the above signatures to be theirs and that they had freely and voluntarily executed this instrument and understood its contents.</td>
                    </tr>
                    <tr>
                            <td  valign="middle" style="height:50px;"><strong>…………………………………………………………………………………<br>Signature & Designation of the person certifying</strong></td><br><br>
                    </tr>
                     <tr>
                            <td  valign="middle" style="height:50px;"><strong> <u>ENTRY INTO ENCUMBRANCES</u><br><br></strong></td>
                    </tr>
                    <tr>
                            <td  valign="left" style="height:50px;">Form of Sub-lease approved Under Section 108 of the Registered Land Act (Now Repealed) Under reference [<strong> '.$buildingmunicipaladd.'</strong> ] and Adopted for use pursuant to Section 108 of the Land Registration Act 2012 to conform with the Land Registration Act 2012 and the Land Act 2012</td><br><br>
                    </tr>
                    <tr>
...
<?php
$sql = "SELECT * FROM draft_document1 WHERE section='lease' AND grouptenantmasid=".$_GET['grouptenantmasid']." AND tenantmasid=".$tenamentvalue." ORDER BY draftid DESC LIMIT 1";
