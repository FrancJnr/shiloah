...
if($load == "loadTenant")
{
  ...
  /*  $sql = "select 
       a.tenantmasid,a.tenanttypemasid, a.leasename, a.tradingname,a.renewalfromid, a.tenantcode, 
       a.companymasid, a.buildingmasid, a.blockmasid, a.floormasid, a.shopmasid, a.shoptypemasid, 
       a.orgtypemasid, a.nob, a.agemasidlt, a.agemasidrc, a.agemasidcp, a.creditlimit, a.latefeeinterest, 
       a.doo, a.doc, a.pin, a.regno, a.address1, a.address2, a.city, a.state, a.pincode, a.country, a.poboxno,
       a.telephone1, a.telephone2, a.fax, a.emailid, a.website, a.remarks, a.createdby, a.createddatetime, 
       a.modifiedby, a.modifieddatetime, a.active,b.shopcode from mas_tenant a inner join mas_shop b on
       b.shopmasid = a.shopmasid where a.active= '1' and a.companymasid=$companymasid and a.tenantmasid not in
       (select tenantmasid from trans_offerletter where companymasid = $companymasid) order by leasename asc";*/
	   $sql = "select 
       a.tenantmasid,a.tenanttypemasid, a.leasename, a.tradingname,a.renewalfromid, a.tenantcode, 
       a.companymasid, a.buildingmasid, a.blockmasid, a.floormasid, a.shopmasid, a.shoptypemasid, 
       a.orgtypemasid, a.nob, a.agemasidlt, a.agemasidrc, a.agemasidcp, a.creditlimit, a.latefeeinterest, 
       ...
       a.modifiedby, a.modifieddatetime, a.active,b.shopcode from mas_tenant a inner join mas_shop b on
       b.shopmasid = a.shopmasid where a.active= '1' and a.companymasid=$companymasid and a.tenantmasid not in 
	   (select c.tenantmasid from mas_tenant c inner join group_tenant_mas b on b.tenantmasid=c.tenantmasid where b.grouptenantmasid not in (select grouptenantmasid from waiting_list)  and c.tenantmasid in(select tenantmasid from trans_offerletter)) order by leasename asc";
}
elseif($load == "loadOfferLetter")
{
  
/* $c="CASE WHEN Country IN ('USA', 'Canada') THEN 'North America'
         WHEN Country = 'Brazil' THEN 'South America'
         ELSE 'Europe' END AS Continent
		 SELECT CASE WHEN COALESCE(@value,'')='' 
    THEN 'Yes, it is null or empty' ELSE 'No, not null or empty' 
    END AS IsNullOrEmpty"; */
  $c	="SELECT CASE WHEN COALESCE(b.leasename, '')='' THEN b.tradingname ELSE b.leasename END AS leasename";
...
elseif($load == "detailsTenant")
{
   $sql = "SELECT n.cpname,a.leasename,a.tradingname,a.tenantcode, a.pincode,a.poboxno, a.city, a.active,DATE_FORMAT( a.doo, \"%d-%m-%Y\" ) AS doo, DATE_FORMAT( a.doc, \"%d-%m-%Y\" ) AS doc, b.age AS term, b1.age AS period,c.buildingname, d.blockname, e.floorname, f.shopcode, f.size\n"
	...
	//SELECT n.cpname,a.leasename,a.tradingname,a.tenantcode, a.pincode,a.poboxno, a.city, a.active,DATE_FORMAT( a.doo, "%d-%m-%Y" ) AS doo, DATE_FORMAT( a.doc, "%d-%m-%Y" ) AS doc, b.age AS term, b1.age AS period,c.buildingname, d.blockname, e.floorname, f.shopcode, f.size FROM mas_tenant a INNER JOIN mas_age b ON b.agemasid = a.agemasidlt LEFT OUTER JOIN mas_tenant_cp n ON n.tenantmasid = a.tenantmasid INNER JOIN mas_age b1 ON b1.agemasid = a.agemasidrc INNER JOIN mas_building c ON c.buildingmasid = a.buildingmasid INNER JOIN mas_block d ON d.blockmasid = a.blockmasid INNER JOIN mas_floor e ON e.floormasid = a.floormasid INNER JOIN mas_shop f ON f.shopmasid = a.shopmasid WHERE n.documentname='1' and a.tenantmasid =1207 and a.companymasid=
...
elseif($load == "detailsOfferLetter")
{
  $sql = "SELECT n.cpname,a.leasename,a.tradingname,a.tenantcode ,a.pincode,a.poboxno, a.city, a.active,DATE_FORMAT( a.doo, \"%d-%m-%Y\" ) AS doo, DATE_FORMAT( a.doc, \"%d-%m-%Y\" ) AS doc, b.age AS term, b1.age AS period,c.buildingname, d.blockname, e.floorname, f.shopcode, f.size\n"
    ...
    . "INNER JOIN mas_floor e ON e.floormasid = a.floormasid\n"
    . "INNER JOIN mas_shop f ON f.shopmasid = a.shopmasid\n"
    . "WHERE t.offerlettermasid =".$load= $_GET['itemval']
...
//	       }
    ...
    $custom=array('msg'=>date('d-F-Y'), 's'=>"Success");
    ...
    if($result != null) 
    {
        ...
        if($cnt > 0)
        {
	      ...
	      //  echo json_encode(utf8ize($arr));
            ...
            echo '{
                "myResult":'.json_encode(utf8ize($arr)).',
                "error":'.json_encode($response_array).
            '}';
        }
        else
