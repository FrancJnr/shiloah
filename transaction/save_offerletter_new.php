...
$sqli ="update mas_tenant set agemasidrc =".$_GET['rentcycle']." where tenantmasid=".$tenantmasid." and companymasid=".$companymasid;
////echo $sqli;
mysql_query($sqli);
...
$sqliii ="select  *  from  mas_tenant where tenantmasid=".$tenantmasid;
$resiii=mysql_query($sqliii);
if($resiii != null)
{
 $rowiii= mysql_fetch_array($resiii);
 $isrenewal=$rowiii['renewal'];
}
if($result != null)
{
...
if($_GET['term'] != null||$_GET['term']!="")
{
    ...
    $termCycle1 = $term[0];
    $termCycle2= $term[1];
   // $termCycle=($termCycle1*12)+$termCycle2;
    $termCycle=$termCycle1+$termCycle2;
    }else{
     $termCycle=$term[0];
...
if($action == "Save")
{
	$createdby = $_SESSION['myusername'];
        ...
        $key ="";
        unset($_GET['term']);
        $sqlAutoNod = "SELECT age FROM mas_age WHERE agemasid=".$_GET['rentcycle'];
$resultx=mysql_query($sqlAutoNod);


if($resultx != null)
{

    $rowzx = mysql_fetch_array($resultx);
    $rentcycle = $rowzx['age'];
...
//		 }
	...
	$cols[] = "companymasid";
     ...
     // echo "<br>OFFER: ".implode($cols, ',')."   ".implode($vals,',')."";
//$sqlGet ="";
//$nk =0;
//foreach ($_GET as $k=>$v) {
                    ...
                    if($val !="")
		    {
				if($j ==0)
				{
				      ...
				      //$vals[] = "'".date('Y-m-d', strtotime($val))."'";
					   $vals[] = "'".date_format(date_create_from_format('d-m-Y', $val), 'Y-m-d')."'";
				}
				else if($j ==1)
				{
				     $cols[] = "ToDate";
				...
				else if($j ==3)
				{
				   if(($termCycle1>=$k)&&($termCycle1>0)){
				     ...
				     $vals[] = "'year'";
                                   ...
                                   }else if(($termCycle2<$k)&&($termCycle2>0)){
			 ...
			 if($j == 4)
			 {
			    ...
			    //echo "<br> RENT: ".implode($cols, ',')."   ".implode($vals,',')."<br>";     
			     ...
			     $j=0;
			     $k++;
                             //echo "TermCycle: ".$termCycle."  Statement: ".$k;
			     if($k > $termCycle)
			    {
                              //echo "mwisho";
				$sqlArray[] = $sqldet;
...
//			     $vals[] = "'".$val."'";
			 ...
			 if($j ==4)
			 {
			     ...
			     {
				$sqldet .='INSERT INTO `'.$table_det.'`('.implode($cols, ',').') VALUES('.implode($vals, ',').');';
			      // echo "<br>SC: ".implode($cols, ',')."   ".implode($vals,',')."<br>";   
	...
	$sqldet .='INSERT INTO `'.$table_det.'`('.implode($cols, ',').') VALUES('.implode($vals, ',').');';
         //echo "<br> DEPO: ".implode($cols, ',')."   ".implode($vals,',')."<br>";   
...
//        exit;
	...
	{
	    $custom = array('isrenewal'=>$isrenewal,'msg'=>$m,'s'=>"Success");    
	}
        ...
        foreach($_GET as $c => $v)
        {
		   ...
		   if($c == "hidFromdateRent".$k)
			...
			//$v = "'".date('Y-m-d', strtotime($v))."'";
			 $v = "'".date_format(date_create_from_format('d-m-Y', $v), 'Y-m-d')."'";
