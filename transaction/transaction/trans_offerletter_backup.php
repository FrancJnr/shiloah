	...
	function calcdeposit5(a)
	{
                            ...
                            if( v== false)
		{
			...
			var c = $('#leegalfeevat').val();
                         var c=Math.round(parseFloat(removeComma(c))/3);
                ...
                if( v== false)
		{
			var b = parseFloat(removeComma($(a).val()));
			var c = $('#leegalfeevat').val();
                        var c=Math.round(parseFloat(removeComma(c))/12);
			var d = Math.round(((b*12*(c/100))*1.14));
			d = d.toFixed(2);
			$('#deposit5').val(commafy(d));
		}
...
//			
                                      ...
                                      if(response.msg=="Data Saved Successfully"){  
                                        var a = confirm("Offer Letter Created Successfully\nWould You Like to Proceed to Group Offer Letter? ");
                                            if (a== true)
                                            {
                                              parent.top.$('div[name=masterdivtest]').html("<iframe  src='Group/offerletter.php?action=new' id='the_iframe2' scrolling='yes' width='100%'></iframe>");   
                                            }else{
                                            $('#depositmonthrent').val('3');
                                            $('#depositmonthsc').val('3');
                                            $('#advancemonthrent').val('14');
	...
	function clearForm()
	{
		...
		if(j>y-1){
                $tbl += "<td>"+Date.parse(Date.parse(frmdt).addYears(y).toString('d-M-yyyy')).addMonths(j-y).toString('d-M-yyyy')+"<input type='hidden' id='hidFromdateRent"+j+"' name='hidFromdateRent"+j+"' value="+Date.parse(Date.parse(frmdt).addYears(y).toString('d-M-yyyy')).addMonths(j-y).toString('d-M-yyyy')+"> </td>";
                $tbl += "<td>"+Date.parse(Date.parse(frmdt).addYears(y).toString('d-M-yyyy')).addMonths(j-y+1).toString('d-M-yyyy')+"<input type='hidden' id='hidTodateRent"+j+"' name='hidTodateRent"+j+"' value="+Date.parse(Date.parse(frmdt).addYears(y).toString('d-M-yyyy')).addMonths(j-y+1).toString('d-M-yyyy')+"> </td>";
                 } else{
                     
              
                $tbl += "<td>"+Date.parse(frmdt).addYears(j).toString('d-M-yyyy')+"<input type='hidden' id='hidFromdateRent"+j+"' name='hidFromdateRent"+j+"' value="+Date.parse(frmdt).addYears(j).toString('d-M-yyyy')+"> </td>";
                $tbl += "<td>"+Date.parse(frmdt).addYears(j+1).toString('d-M-yyyy')+"<input type='hidden' id='hidTodateRent"+j+"' name='hidTodateRent"+j+"' value="+Date.parse(frmdt).addYears(j+1).toString('d-M-yyyy')+"> </td>";
...
//               
               ...
               for (var j=0; j<x; j++){  
                $tbl  = "<tr>";
		if(j>y-1){          
                $tbl += "<td>"+Date.parse(Date.parse(frmdt).addYears(y).toString('d-M-yyyy')).addMonths(j-y).toString('d-M-yyyy')+"<input type='hidden' id='hidFromdateSc"+j+"' name='hidFromdateSc"+j+"' value="+Date.parse(Date.parse(frmdt).addYears(y).toString('d-M-yyyy')).addMonths(j-y).toString('d-M-yyyy')+"> </td>";
                $tbl += "<td>"+Date.parse(Date.parse(frmdt).addYears(y).toString('d-M-yyyy')).addMonths(j-y+1).toString('d-M-yyyy')+"<input type='hidden' id='hidTodateSc"+j+"' name='hidTodateSc"+j+"' value="+Date.parse(Date.parse(frmdt).addYears(y).toString('d-M-yyyy')).addMonths(j-y+1).toString('d-M-yyyy')+"> </td>";
                 } else{
                $tbl += "<td>"+Date.parse(frmdt).addYears(j).toString('d-M-yyyy')+" <input type='hidden' id='hidFromdateSc"+j+"' name='hidFromdateSc"+j+"' value="+Date.parse(frmdt).addYears(j).toString('d-M-yyyy')+"> </td>";
                $tbl += "<td>"+Date.parse(frmdt).addYears(j+1).toString('d-M-yyyy')+" <input type='hidden' id='hidTodateSc"+j+"' name='hidTodateSc"+j+"' value="+Date.parse(frmdt).addYears(j+1).toString('d-M-yyyy')+"> </td>";
                 }   
                if(j == 0){
...
//
//          var d = Date.parse(frmdate).addYears(1).toString('d-M-yyyy');
//          var d2 = Date.parse(frmdate).addMonths(12).toString('d-M-yyyy');
//          alert(d.toString('d-M-yyyy'));
//          
//        if(period=="years"){
//         var todt=d;
